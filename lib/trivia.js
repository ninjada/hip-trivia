...
Trivia.prototype.checkAnswer = function* (guess) {
	...
	if (tenant.getState('mode') == modes.ACTIVE && answer) {
		...
		if (correct) {
			...
			yield this.sendMessage('${this.sender.name} is correct! ${utils.generateScore(record)}<br />${streakMsg}<br />Answer: <b>${tenant.getState(`answer`)}</b>', {notify: true, color: 'green'});
